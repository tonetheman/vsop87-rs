...
#[inline]
#[cfg(feature = "f32")]
fn calculate_t(jde: f32) -> f32 {
    return (jde - 2451545_f32) / 365250_f32;
}
...
#[inline]
#[cfg(feature = "f32")]
fn calculate_var(t: f32, var: &[(f32, f32, f32)]) -> f32 {
    var.iter().fold(0_f32, |term, &(a, b, c)| term + a * (b + c * t).cos())
}
...
/// ```
#[cfg(feature = "f64")]
pub fn keplerian_elements_from_vsop87(a: f64,
                                      l: f64,
                                      k: f64,
                                      h: f64,
                                      q: f64,
...
pub fn mercury(jde: f32) -> (f32, f32, f32, f32, f32, f32) {
    ...
    let a = a0 + a1 * t + a2 * t * t;
    let l = (l0 + l1 * t + l2 * t * t + l3 * t.powi(3)) % (2_f32 * PI);
    let k = k0 + k1 * t + k2 * t * t + k3 * t.powi(3) + k4 * t.powi(4) + k5 * t.powi(5);
    let h = h0 + h1 * t + h2 * t * t + h3 * t.powi(3) + h4 * t.powi(4) + h5 * t.powi(5);
    let q = q0 + q1 * t + q2 * t * t + q3 * t.powi(3) + q4 * t.powi(4) + q5 * t.powi(5);
    let p = p0 + p1 * t + p2 * t * t + p3 * t.powi(3) + p4 * t.powi(4);
...
/// ```
#[cfg(feature = "f32")]
pub fn venus(jde: f32) -> (f32, f32, f32, f32, f32, f32) {
    let t = calculate_t(jde);
...
/// ```
#[cfg(feature = "f32")]
pub fn earth_moon(jde: f32) -> (f32, f32, f32, f32, f32, f32) {
    let t = calculate_t(jde);
...
/// ```
#[cfg(feature = "f32")]
pub fn mars(jde: f32) -> (f32, f32, f32, f32, f32, f32) {
    let t = calculate_t(jde);
...
/// ```
#[cfg(feature = "f32")]
pub fn jupiter(jde: f32) -> (f32, f32, f32, f32, f32, f32) {
    let t = calculate_t(jde);
...
/// ```
#[cfg(feature = "f32")]
pub fn saturn(jde: f32) -> (f32, f32, f32, f32, f32, f32) {
    let t = calculate_t(jde);
...
/// ```
#[cfg(feature = "f32")]
pub fn uranus(jde: f32) -> (f32, f32, f32, f32, f32, f32) {
    let t = calculate_t(jde);
...
/// ```
#[cfg(feature = "f32")]
pub fn neptune(jde: f32) -> (f32, f32, f32, f32, f32, f32) {
    let t = calculate_t(jde);
